...
const CustomMobileLink = ({className="", href="", title, toggle }) => {
  ...
  return(
    ...
    onClick={handleClick}>
      {title}
      <span className={`h-[1px] inline-block w-0 bg-light dark:bg-dark absolute left-0 -bottom-[2px] group-hover:w-full transition-[width] ease duration-300 
      ${router.asPath === href ? 'w-full' : 'w-0'}`}>
        &nbsp;
      </span>
    </button>
...
const NavBar = () => {
  ...
  return (
    <header className='z-10 fixed w-full px-32 md:px-16 sm:px-8 py-4 font-medium flex items-center justify-between dark:text-light bg-light dark:bg-dark border-b-2 md:bg-light/70
    ...
    <button className=' flex-col justify-center items-center hidden lg:flex ' onClick={handleClick}>
        ...
        
      {
        ...
        backdrop-blur-md py-32'>
          ...
          <CustomMobileLink href="#activities" title="Activities" className='' toggle={handleClick}/>
          <CustomMobileLink href="#projects" title="Projects" className='' toggle={handleClick}/>
          </nav>
          <nav className='flex items-center justify-center flex-wrap mt-4'>
            <motion.a href="https://www.facebook.com/kinekine41" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:0.8}} className='w-8 mx-3 bg-myblue sm:mx-1'><FacebookIcon /></motion.a>
            <motion.a href="https://github.com/kyle41io" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:0.8}} className='w-8 mx-3 sm:mx-1 bg-light dark:bg-dark rounded-full'><GithubIcon /></motion.a>
            <motion.a href="https://www.linkedin.com/in/kyleincontact" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:0.8}} className='w-7 mx-3 sm:mx-1 '><LinkedInIcon /></motion.a>
    
            <button onClick={() => setMode( mode === "light" ? "dark" : "light")}
            className={`ml-5 sm:ml-2 flex items-center justify-center rounded-full p-1.5 ${mode === "light" ?"bg-dark text-light" : "bg-light text-dark"}`}>
              {mode === "dark" ? <MoonIcon className={"fill-dark"} /> :
              <SunIcon className={"fill-dark"} />}
            </button>
